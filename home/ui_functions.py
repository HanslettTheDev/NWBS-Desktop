...
class InterfaceTweaks(BaseHomeWindow):
	...
	def real_database(self):
		...
		if reply == QMessageBox.StandardButton.Yes:
			if not create_database():
				QMessageBox.information(None, "Notification", "Database Error. Something went wrong")
			QMessageBox.information(None, "Notification", "Database Succesfully Created")
	...
	def _after_home_view_load(self):
		...
		if reply and input_.text() != "":
			...
			if not save_name:
				return QMessageBox.information(None, "Notification", "Congregation Name Saved")
			self.ui_tweaks._after_congregation_check(self)
		else:
