...
class ProgramDialog(QDialog):
    current_index = 1
    def __init__(self, parent=None, month="", _dict:dict={}, count:int = 0, pre_data:dict = {}, load=True):
    ...
    def setup_ui(self, _dict):
        ...
        bible_reading = QLabel(f"Bible Reading")
        ...
        form.addRow(hall2)
        form.addRow(QLabel(f"Bible Reading"), self.bible_reading_input_2)
        form.addRow(live_as)
        ...
        self.btn1 = QPushButton("Load Previous File")
        self.btn1.clicked.connect(self.load)
        form.addWidget(self.button_box)
        form.addWidget(self.btn1)
    ...
    def load(self):
        # load a previous json data
        with open(
            os.path.join(os.getcwd(), FOLDER_REFERENCES["generated_programs"], self.month +" program.json"), 
            "r") as file:
            previous_info = json.load(file)
        self.populate_data(previous_info[self._dict['month']])

    def populate_data(self, previous_info):
        # Set the respective text data for the previous data
        ...
        self.bible_reading_input.setText(previous_info["bible_reading"])
        self.bible_reading_input_2.setText(previous_info["bible_reading_secondhall"])
        # Grab all the widgets
        preaching_widgets = [widget for widget in self.findChildren(QLineEdit) if widget.objectName().startswith("input_preaching_")]
        secondhall_widgets = [widget for widget in self.findChildren(QLineEdit) if widget.objectName().startswith("input_preaching_secondhall_")]
        las_widgets = [widget for widget in self.findChildren(QLineEdit) if widget.objectName().startswith("input_middle_parts_")]
